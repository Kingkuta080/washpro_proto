...
const GutterDashboard = () => {
  ...
  const handleGetGutterById = async (id: string) => {
    setIsLoading(true);
    try {
      const data = await apiController.get(`gutters/${id}`);
  ...
  const handleCreateGutter = async (formData: any) => {
    setIsLoading(true);
    try {
      await apiController.post('gutters', formData);
      setAlert({
        variant: 'success',
  ...
  const handleUpdateGutter = async (id: string, formData: any) => {
    setIsLoading(true);
    try {
      await apiController.put(`gutters/${id}`, formData);
      setAlert({
        variant: 'success',
  ...
  const handleDeleteGutter = async (id: string) => {
    setIsLoading(true);
    try {
      await apiController.delete(`gutters/${id}`);
      setAlert({
        variant: 'success',
