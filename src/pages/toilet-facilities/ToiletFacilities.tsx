...
const ToiletStats = () => {
  ...
  const [newToiletData, setNewToiletData] = useState({
    type: '',
    count: 0,
    status: '',
  });
  ...
  const fetchToiletTypes = async () => {
    setLoading(true);
    try {
      const response = await apiController.get('/toilet-types'); // Adjust the endpoint as needed
      setToiletTypes(response.data || []);
    } catch (error) {
  ...
  const handleUpdateToilet = async () => {
    ...
    try {
      setLoading(true);
      const response = await apiController.put(`/toilet-types/${selectedToilet._id}`, newToiletData);
  ...
  const handleDeleteToilet = async () => {
    ...
    try {
      setLoading(true);
      const response = await apiController.delete(`/toilet-types/${selectedToilet._id}`);
